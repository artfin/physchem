...
Lines <- list(bquote(paste("Зависимость фактора эффективности ", eta[RP] , " от модуля Тилле ", phi[r], " при ")),
              bquote(paste("геометрических параметрах ", beta, "= 1.75, ", delta, "= 0.23")))
...
plot(x, func(x, 50, 50, 50), type = "l", col = "darkorchid", lwd = 2, 
     xlab = expression(eta[RP]), ylab = expression(phi[r]))
Lines <- list(bquote(paste("Зависимость фактора эффективности ", eta[RP] , " от модуля Тилле ", phi[r], " при ")),
              bquote(paste("геометрических параметрах ", beta, "= 2.25, ", delta, "= 0.9")))
mtext(do.call(expression, Lines),side=3,line=1:0)
